...
// solutuion to a problem  = algorithms
...
const prices = [10, 50, 0, 100];
// -> array.find() returns the first element that satisfies some condition.
prices.find (price => price >25);
//can name it to refernce later
const firstHighPrice =prices.filter(price => price <50)
...
// prices.reduce({accumlator, element});
// 1. sum = 0, price is prices(0) = 10, sum = sum + price = 0+10 = 10
// 2. sum = 10 , price = prices (1) = 50, sum = sum + price = 10 + 50 = 60
// 3. sum = 60 , price = prices (2) = 
...
const returnedQuestion = phrases.filter(phrase => phrase.includes("?"))
